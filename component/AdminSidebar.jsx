...
function AdminSidebar(props) {
  ...
  useEffect(() => {
    {
      router.pathname.split("/").at(-1) === "dashboard"
        ? setDashboard("active")
        : setDashboard("unactive");
    }
    {
      router.pathname.split("/").at(-1) === "history"
        ? setHistory("active")
        : setHistory("unactive");
    }
    {
      router.pathname.split("/").at(-1) === "users"
        ? setUsers("active")
        : setUsers("unactive");
    }
    {
      router.pathname.split("/").at(-1) === "donations"
        ? setDonation("active")
        : setDonation("unactive");
    }
  }, []);
      ...
      <div className="d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 text-white min-vh-100">
        ...
        <ul className="nav nav-pills flex-column mb-auto">
          ...
          <li className="nav-item">
            <a href="users" className={"nav-link text-white " + users}>
              <i aria-hidden className="fas fa-hand-holding-medical"></i> &nbsp;
              Users
            </a>
          </li>
          <li className="nav-item">
        ...
        <div className="dropdown pb-4">
          ...
          >
            ...
            <span className="d-none d-sm-inline mx-1">{props.name}</span>
          </a>
        </div>
      </div>
    </div>
